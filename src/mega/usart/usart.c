...
/*
 ...
 |                            < UARTDataFramError0  >                                                          |
  ---------------------------------------------------------------------------------------------------------
 | < @Function          : uint8_t UARTDataFramError0                                                           |
 | < @Description       : Frame Error Detection                                        |     
 | < @return            : 1 no Frame error , 0 Frame error                                               |
  ---------------------------------------------------------------------------------------------------------
 */
uint8_t UARTDataFramError0() {
    uint8_t state = UART0_STATUS;
    if (state & ((1 << FE) | (1 << DOR))) {
       UART0_STATUS = 0x00;
        return (1);
    }
...
/*
 ...
 |                            < UARTDataFramError1  >                                                          |
  ---------------------------------------------------------------------------------------------------------
 | < @Function          : uint8_t UARTDataFramError1                                                           |
 | < @Description       : Frame Error Detection                                        |     
 | < @return            : 1 no Frame error , 0 Frame error                                               |
  ---------------------------------------------------------------------------------------------------------
 */
uint8_t UARTDataFramError1() {
    uint8_t state = UART1_STATUS;
    if (state & ((1 << FE1) | (1 << DOR1) | (1 << PE1))) {
        return (1);
    }
    return (0);
}
