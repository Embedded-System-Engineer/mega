...
void PrintDate(tm_t *tm, uint8_t *buf) {
    print2d(tm->tm_mday, buf, 0);
    buf[2] = ' ';
    memcpy_P((char *) (buf + 3), (monthsOfTheYear + tm->tm_mon - 1), 3);
    buf[6] = ' ';
    buf[7] = '2';
    buf[8] = '0';
    print2d(tm->tm_year, buf, 9);
}


/**************************************************************************/
/*!   @brief  Print Date 10 MAR 00:00 FRI     
 *    
 */

/**************************************************************************/
void PrintDateTime(tm_t *tm, uint8_t *buf) {
    setBuff(buf, ' ', PRINT_BUF_DATETIME_lENGTH);
    print2d(tm->tm_mday, buf, 0);
    memcpy_P((char *) (buf + 3), (monthsOfTheYear + tm->tm_mon - 1), 3);
    print2d(tm->tm_hour, buf, 7);
    if (systemReadHalfSecond()) {
        buf[9] = ':';
    }
    print2d(tm->tm_min, buf, 10);
    memcpy_P((char *) (buf + 13), dOfTheWeek + tm->tm_wday, 3);
